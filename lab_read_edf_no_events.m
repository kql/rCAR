% lab_read_edf_no_events() - read eeg data in EDF+ format, but without events
% (modified by Giorgio)
%
% Orignal Code-file:
...
function [data,header,cfg] = lab_read_edf_no_events(filename,cfg)
...
haveref = false;
while m > 1
    if ~isempty(strfind(upper(header.channels(m,:)),'XXX')) | ~isempty(strfind(upper(header.channels(m,:)),'XXX'))  % it was written 'ECG' and 'EKG'
        extrachannel(1,m) = 1;
        header.ecg_ch = m;
    elseif ~isempty(strfind(upper(header.channels(m,:)),'XXX')) % it was written 'EOG'
        extrachannel(1,m) = 1;
        header.eog_ch = m;
    elseif ~isempty(strfind(upper(header.channels(m,:)),'XXX')) % it was written 'PHOTIC'
        extrachannel(1,m) = 1;
    elseif ~isempty(strfind(upper(header.channels(m,:)),'XXX')) & ~strcmpi(header.channels(m,1:3),'XXX') % it was written 'REF' and 'REF'
        includeref = true;
    elseif ~isempty(strfind(upper(header.channels(m,:)),'XXX')) & strcmpi(header.channels(m,1:3),'XXX') % it was written 'REF' and 'REF'
        haveref = true;
    end
...
if includeref == true & haveref == false
    ...
    header.channels(end+1,1:3) = 'REF'; % it was written 'REF' '
